...
// ReviewPrompt asks the reviewer for this round's verdict. The instructions narrow as rounds go
// on, so a loop converges on regressions instead of relitigating settled low-severity findings.
...
// FixPrompt asks the author to act on the review and record what it decided. It narrows the same
// way ReviewPrompt does, and always demands the decision record: that record is how the next
// round knows what has already been settled.
func FixPrompt(reviewPath, summaryPath string, iteration int) string {
	...
	return fmt.Sprintf("You are the author in round %d of an automated review loop and have a fresh session. Read %s, then read %s if it exists and inspect the current uncommitted diff.\n\n%s Do not edit %s.\n\nBefore finishing, rewrite %s as a compact decision record: keep at most 20 bullets, each beginning with applied:, rejected:, or deferred: and carrying a short reason. Retain only current applied decisions and still-relevant rejected or deferred ones. End the file with a single line beginning with tests: that records whether you ran the build and the test suite and what the outcome was. The next reviewer will use this record to avoid repeating settled comments.", iteration, reviewPath, summaryPath, policy, reviewPath, summaryPath)
}
